        ...
        private void green_Print(object sender, PrintPageEventArgs e)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    ...
                    e.Graphics.DrawString("2017/11/25/Manabu Hano", font, brush, new Point(datex + i * minx, datey + j * miny));
                }
            }


            //e.Graphics.DrawString("テスト123456789", font, brush, new PointF(-500, 0));//←、↑

            //Pen p = new Pen(Color.Black);
            //e.Graphics.DrawRectangle(p, new Rectangle(20, 100, 200, 100));//(20,100) 200*100
            font.Dispose();
        }
